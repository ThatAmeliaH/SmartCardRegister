...
object JSONHandler {
   ...
   * @throws NullPointerException if name or studentId are null.
   */
  @throws[NullPointerException]
  @NotNull def CreateStudentJSON(@NotNull name: String, @NotNull studentId: String, @Nullable UID: String): JSONObject = {
    if (name == null || studentId == null) { throw new NullPointerException() }

    val studentMap: java.util.Map[String, String] = Map(
      "name" -> name,
      "id" -> studentId,
      "UID" -> (if (UID != null) UID else "")
    ).asJava
   ...
   */
  @NotNull def ToJSONArray(@NotNull objects: Array[JSONObject]): JSONArray = {
   ...
   */
  @NotNull def ToJSONString(@Nullable array: JSONArray, @NotNull indentFactor: Int): String = {
    if (array == null) "[]"
   ...
   */
  @NotNull def ParseJSONArray(@Nullable jsonString: String): JSONArray = {
